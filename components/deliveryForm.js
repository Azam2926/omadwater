...
export default function DeliveryForm() {
  ...
  return isSubmitSuccessful ? (
    ...
    <form className={'grid items-center space-y-4'} onSubmit={handleSubmit(sendForm)}>
      <input {...register('ismi', {pattern: /^[A-Za-z']+$/i})} className={'focus:ring-2 ring-blue-500 outline-none py-3 px-6 border rounded-full mx-auto w-full'} required type="text" placeholder={'Ismingiz'}/>
      ...
      <button disabled={isSubmitting} className={'flex hover:bg-black bg-omad-btn shadow-omad font-semibold outline-none cursor-pointer text-white rounded-full m-auto py-btnY px-btnX'} type="submit">
        {isSubmitting ? (
